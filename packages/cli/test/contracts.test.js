...
test("the published command and flag surface has not changed", async () => {
  ...
  assert.deepEqual(surface, {
    ...
    // changes meaning, and the panel it uncovers -- the method, the percentile behind the pressure
    // band, the policy versions -- is what moving those out of the default reading required. The
    // same flag already existed on `stats`, so the two commands spell the same idea the same way.
